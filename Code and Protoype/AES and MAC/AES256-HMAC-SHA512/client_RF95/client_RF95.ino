...
void loop()
{
  ...
  macGenerator(&sha512,(const uint8_t*)pesan, key, result);
  ...
  Serial.print("MESSAGE + HMAC-SHA512 : ");
  printData(data, len);
  ...
  if (rf95.waitAvailableTimeout(5000))
  {
    ...
    if (rf95.recv(buf, &pjg))
    {
      ...
      Serial.print("HMAC-SHA512       : ");
      printData(rmac, HASH_SIZE);

      // Verify MAC
      macVerify(&sha512,(const uint8_t*)rmsg, key, rmac);
